module EndpointControllerTests
...
let testSubController (id: int) = subcontroller {
...
// let testHost = hostFromController testController

[<Tests>]
let routingTests =
    let responseTestCaseDefault () = responseEndpointTestCase (hostFromController testController)

    testList "Endpoint Controller Routing Tests" [
        ...
        testCase "deleteAll works" <| fun _ ->
            ...
            let deleteHost = hostFromController deleteController
            let ctx = getEmptyContext "DELETE" ""
            let res = testHostWithContext deleteHost ctx

            Expect.equal (res.Response.StatusCode) expectedStatusCode "Status code should be 204"
        ...
        testCase "Request `/test/1` returns `test` as controller's key handlers should match up to one path segment" <|
          responseEndpointTestCase (hostFromController testPathSegmentController) "PUT" "/test" "Update test"
...
let implicitStringToHtmlTestController = controller {
   index (fun _ -> task { return RenderView.AsString.htmlNode basicTemplate})
}
...
let routingStringTests =
    let responseTestCaseDefault () = responseEndpointTestCase (hostFromController testStringController)

    testList "Endpoint Controller Routing with String Tests" [
